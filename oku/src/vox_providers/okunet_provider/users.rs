...
impl OkuNetProvider {
    pub async fn get_user_frontmatter(
        ...
        if let Some(identity) = user.identity.clone() {
            for followed_user in identity.following {
                ...
                followed_user_table.insert(
                    "id".into(),
                    oku_core::fs::util::fmt(followed_user_information.author_id).into(),
                );
        ...
        table.insert(
            "permalink".into(),
            format!("{}.html", oku_core::fs::util::fmt(user.author_id)).into(),
        );
        table.insert("title".into(), user_name.into());
        table.insert(
            "author_id".into(),
            oku_core::fs::util::fmt(user.author_id).into(),
        );
        table.insert(
            "is_followed".into(),
        ...
        if !posts.is_empty() {
            table.insert(
                "depends".into(),
                vec![oku_core::fs::util::fmt(user.author_id)].into(),
            );
        } else {
    ...
    pub async fn create_profile_page(
        ...
        let user_posts = posts.unwrap_or(
            oku_core::database::core::DATABASE
        ...
        let page_path = format!("{}.vox", oku_core::fs::util::fmt(user.author_id));
        let include_argument = if !user_posts.is_empty() {
            oku_core::fs::util::fmt(user.author_id)
        } else {
            "empty".into()
        };
...
",
    ...
    pub async fn view_self(&self) -> miette::Result<String> {
        ...
        self.render_and_get(format!(
            "output/{}.html",
            oku_core::fs::util::fmt(me.author_id)
        ))
    }
}
