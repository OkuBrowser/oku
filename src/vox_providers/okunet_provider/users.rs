...
impl OkuNetProvider {
    pub async fn get_user_frontmatter(
        ...
        let user_name = match &user.identity {
            ...
            None => oku_fs::fs::util::fmt(user.author_id),
        };
        let node = NODE
            .get()
        ...
        if let Some(identity) = user.identity.clone() {
            for followed_user in identity.following {
                ...
                followed_user_table.insert(
                    "id".into(),
                    oku_fs::fs::util::fmt(followed_user_information.author_id).into(),
                );
        ...
        table.insert(
            "permalink".into(),
            format!("{}.html", oku_fs::fs::util::fmt(user.author_id)).into(),
        );
        ...
        if !posts.is_empty() {
            table.insert(
                "depends".into(),
                vec![oku_fs::fs::util::fmt(user.author_id)].into(),
            );
        } else {
    ...
    pub async fn create_profile_page(
        ...
        let page_path = format!("{}.vox", oku_fs::fs::util::fmt(user.author_id));
        let include_argument = if !user_posts.is_empty() {
            oku_fs::fs::util::fmt(user.author_id)
        } else {
            "empty".into()
        };
...
",
    ...
    pub async fn view_self(&self) -> miette::Result<String> {
        ...
        self.render_and_get(format!(
            "output/{}.html",
            oku_fs::fs::util::fmt(me.author_id)
        ))
    }
}
