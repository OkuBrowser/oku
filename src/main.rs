...
async fn main() {
    ...
    let local_directory = format!("{}/{}", project_directories.cache_dir().to_str().unwrap(), test);
    let mut hierarchy = HashMap::new();
    hierarchy.insert(test.to_owned(), local_directory.to_owned());
    ipfs_download_directory(&client, local_directory.to_owned(), test.to_owned(), hierarchy).await;
}

async fn ipfs_download_file(client: &IpfsClient, file_hash: String, file_path: String)
{
    match client.cat(&file_hash).map_ok(|chunk| chunk.to_vec()).try_concat().await
    {
        Ok(res) => {
        ...
        Err(e) => eprintln!("Failed to obtain file: {} ({})\nError: {:#?}", file_path, file_hash, e)    
    }
}

#[async_recursion(?Send)]
async fn ipfs_download_directory(client: &IpfsClient, directory: String, directory_hash: String, mut hierarchy: HashMap<String, String>)
{
    ...
    for object in directory_object
    {
        for link in object.1.links
        {
            ...
            {
                &"Directory" => {
                    let sub_directory = format!("{}/{}", hierarchy.get(&directory_hash).unwrap(), link.name);
                    ipfs_download_directory(&client, sub_directory.to_owned(), link.hash.clone(), hierarchy.clone()).await;
                    hierarchy.insert(link.hash, sub_directory);
                }
                &"File" => {
                    ipfs_download_file(&client, link.hash, format!("{}/{}", hierarchy.get(&directory_hash).unwrap(), link.name)).await;
                }
                &_ => {
